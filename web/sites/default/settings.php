 ...
 * its location.
 */
// Đường dẫn giải tương đối theo Drupal root (web/), nên trỏ tới config/sync ở
// gốc dự án — ngoài webroot, không truy cập được qua HTTP.
//
// Phải đặt TRƯỚC include settings.ddev.php ở cuối file: DDEV chạy
// if (empty($settings['config_sync_directory'])) và sẽ gán
// 'sites/default/files/sync' nếu thấy rỗng — thư mục đó bị .gitignore chặn.
 ...
 */
// File này (settings.php) ĐƯỢC COMMIT và repo là public — tuyệt đối không đặt
// secret vào đây. Mọi secret (hash_salt, DB credentials, API key,
// trusted_host_patterns của production) đặt trong settings.local.php.
// File đó bị .gitignore chặn và nằm cuối cùng nên ghi đè được mọi giá trị trên.
